...
class Board:
    ...
    def neighbours_of(self, stone):
        ...
        neighbours={(stone[0]+1,stone[1]),(stone[0]-1,stone[1]),(stone[0],stone[1]+1),(stone[0],stone[1]-1)}
        not_on_board = set()
    ...
    def is_on_board(self, stone):
        if stone[0]>=self.size or stone[0]<0 or stone[1]>=self.size or stone[1]<0:
            return False
        else:
            return True
...
matrix = [[0, 0, "b", "w"], ["w", "b", "b", 0], [0, "w", "w", "w"], ["b", "w", "w", "w"]]
